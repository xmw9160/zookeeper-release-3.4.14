...
public class QuorumPeer extends ZooKeeperThread implements QuorumStats.Provider {
    ...
    public synchronized void start() {
        // 回复快照数据
        loadDataBase();
        // 启动zkServer，相当于用户可以通过2181这个端口进行通信了
        // 默认cnxnFactory为NIOServerCnxnFactory
        cnxnFactory.start();
        // 开始leader选举
        startLeaderElection();
        //XXX leader选举流程 逻辑在run()方法中
        super.start();
    }
    ...
    synchronized public void startLeaderElection() {
    	...
    	//XXX getView 返回的就是在配置文件中配置的 server.myid=ip:port:port
    ...
    protected Election createElectionAlgorithm(int electionAlgorithm){
        ...
        case 2:
            ...
            break;
        case 3:
            // QuorumCnxManager 是一个很核心的对象，用来实现领导选举中的网络连接管理功能
            qcm = createCnxnManager();
            QuorumCnxManager.Listener listener = qcm.listener;
            if(listener != null){
                // 启动监听器
                // listener实现了线程，所以在run方法中可以看到构建ServerSocket的请求，
                // 这里专门用来接收其他zkServer 的投票请求
                listener.start();
            ...
            while (running) {
                ...
                case LOOKING:
                    ...
                    if (Boolean.getBoolean("readonlymode.enabled")) {
                        ...
                        try {
                            ...
                            //XXX 默认FastLeaderElection.lookForLeader
                            setCurrentVote(makeLEStrategy().lookForLeader());
                        } catch (Exception e) {
                            LOG.warn("Unexpected exception", e);
                            setPeerState(ServerState.LOOKING);
                ...
                case LEADING:
                    ...
                    try {
                        ...
                        //XXX 初始化一个 Leader 对象，构建一个 LeaderZookeeperServer，用于表示leader 节点的请求处理服务
                        leader.lead();
                        setLeader(null);
                    } catch (Exception e) {
                        LOG.warn("Unexpected exception",e);
     ...
     * getView 里面实际上返回的是一个 quorumPeers，就是参与本次投票的成员有哪些
     */
